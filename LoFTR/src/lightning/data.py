...
from src.datasets.demis import DEMISDataset
...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def __init__(self, args, config):
        ...
        # DEMIS options
        self.demis_img_resize = config.DATASET.DEMIS_IMG_RESIZE  # 640
        self.demis_img_pad = config.DATASET.DEMIS_IMG_PAD   # True
        self.demis_depth_pad = config.DATASET.DEMIS_DEPTH_PAD   # True
        self.demis_df = config.DATASET.DEMIS_DF  # 8
    ...
    def _setup_dataset(self,
        ...
        if scene_list_path is not None:
            ...
            local_npz_names = []
            logger.info(f'[rank {self.rank}]: Skipping scene assignment as no indices were given.')
    ...
    def _build_concat_dataset(
        ...
        # `ScanNetDataset`/`MegaDepthDataset`/`DEMISDataset` load all data from
        # npz_path when initialized, which might take time.
        if str(data_source).lower() in ['megadepth', 'demis']:
        ...
        for npz_name in tqdm(npz_names,
            ...
            elif data_source == 'DEMIS':
                datasets.append(
                    DEMISDataset(data_root,
                                 npz_path,
                                 mode=mode,
                                 img_resize=self.demis_img_resize,
                                 df=self.demis_df,
                                 img_padding=self.demis_img_pad,
                                 depth_padding=self.demis_depth_pad,
                                 coarse_scale=self.coarse_scale))
            else:
